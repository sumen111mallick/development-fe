...
export class ProductpageComponent implements OnInit {
      ...
      this.userService.getUserPhoneDetails().subscribe(
        data => {
          if (data !== 1) {
            ...
            this.showLoadingIndicator = false;
            this.tokenStorage.saveReturnURL('product_payment?productID=' + productId);
            this.openModal(productId);
          }
          else {
            console.log("Mobile number verified");
            this.showLoadingIndicator = false;
            this.router.navigate(['product_payment'], { queryParams: { 'productID': productId } });
          }
        }
      );
    }
  ...
  proceedToPayment(productId) {
    this.router.navigate(['/pro_payment_summary'], { queryParams: {'productID': productId } });
  }

  openLoginModal(product_id) {
